...
namespace EsportsManager.Scripts.MainMenu.Factories
{
    public static class PlayerFactory
    {
        ...
        public static readonly Dictionary<string, (string[] firstNames, string[] lastNames, string[] nickPrefixes, string[] nickSuffixes)> NationalityData = new()
        {
        ...
        public static Player GeneratePlayer(GameState gameState)
        {
            var nationalities = NationalityData.Keys.ToList();
            string nat = nationalities[random.Next(nationalities.Count)];

            return GeneratePlayer(gameState, nat, null);
        }
        
        public static Player GeneratePlayer(GameState gameState, Team team)
        {
            var nationalities = NationalityData.Keys.ToList();
            string nat = nationalities[random.Next(nationalities.Count)];

            return GeneratePlayer(gameState, nat, team);
        }

        public static Player GeneratePlayer(GameState gameState, string nationality, Team? team)
        {
        ...
        public static GameProfile GenerateGameProfile(Player player, Game game)
        {
        ...
        GameProfile profile;
            int potential = random.Next(0, 100);
            int tactics = random.Next(0, 100);
            int mechanics = random.Next(20, 90);
            int reflexes = random.Next(20, 95);
            int communication = random.Next(0, 100);
            int teamwork = random.Next(15, 100);
            int gameSense = random.Next(20, 100);
            int adaptability = random.Next(5, 90);
            int consistency = random.Next(0, 99);

            switch (game.ShortName)
            {
                case "CS":
                    profile = new GameProfile_CS
                    {
                        GameId = game.Id,
                        PlayerId = player.Id,//(Role_CS)random.Next(Enum.GetValues(typeof(Role_CS)).Length), //
                        Role = RoleDecider(potential, tactics, mechanics, reflexes, communication, teamwork, gameSense, adaptability, consistency),
                        Potential = potential,
                        Tactics = tactics,
                        Mechanics = mechanics,
                        Reflexes = reflexes,
                    ...
                    break;
                case "LoL":
                    profile = new GameProfile_LoL
                    {
                        Game = game,
                        Player = player,
                        Role = (Role_LoL)random.Next(Enum.GetValues(typeof(Role_LoL)).Length),
                        Kills = 0,
                        Deaths = 0,
                        Assists = 0,
                        GamesPlayed = 0,
        ...
        private static Role_CS RoleDecider(int potential, int tactics, int mechanics, int reflexes, int communication, int teamwork, int gameSense, int adaptability, int consistency)
        {
            // Normalize values between 0-1
            double norm(int value) => value / 100.0;

            // Role weights
            double awperScore = norm(reflexes) * 0.4 + norm(mechanics) * 0.4 + norm(consistency) * 0.2;
            double lurkerScore = norm(gameSense) * 0.4 + norm(adaptability) * 0.3 + norm(mechanics) * 0.3;
            double supportScore = norm(teamwork) * 0.4 + norm(communication) * 0.4 + norm(adaptability) * 0.2;
            double iglScore = norm(communication) * 0.4 + norm(tactics) * 0.4 + norm(gameSense) * 0.2;
            double riflerScore = norm(mechanics) * 0.4 + norm(reflexes) * 0.3 + norm(tactics) * 0.2 + norm(consistency) * 0.1;

            var scores = new Dictionary<Role_CS, double>
            {
                { Role_CS.AWPer, awperScore },
                { Role_CS.Lurker, lurkerScore },
                { Role_CS.Support, supportScore },
                { Role_CS.InGameLeader, iglScore },
                { Role_CS.Rifler, riflerScore }
            };

            // Get highest-scoring role
            var bestRole = scores.OrderByDescending(kvp => kvp.Value).First().Key;
            return bestRole;
        }
        ...
        private static double CalculateOverallRating(GameProfile_CS stats)
        {
            // Weighted average of stats
            return (stats.Mechanics * 0.3 + stats.Reflexes * 0.25 + stats.GameSense * 0.2 +
                   stats.Teamwork * 0.15 + stats.Consistency * 0.1) / 100.0 * 2.0;
        }

        public static double CalculateMarketValue(Player player, GameProfile_CS stats)
        {
            double potentialFactor = stats.Potential / 100.0;
            double ageFactor = 1.0 - Math.Abs(player.Age - 26) * 0.05; // Peaks at 26
            return (player.Rating > 0 ? player.Rating : 0.1) * 5000 * potentialFactor * ageFactor;
        }

        public static double CalculateMarketValue(GameProfile_LoL stats, int age)
        {
            double potentialFactor = stats.Potential / 100.0;
            double ageFactor = 1.0 - Math.Abs(age - 26) * 0.05; // Peaks at 26
            return (1 * 100000 * potentialFactor * ageFactor);
        }

        public static double CalculateExpectedSalary(Player player,GameProfile_CS stats)
        {
            return CalculateMarketValue(player, stats) / 20; // Salary is 5% of market value
        }

        public static double CalculateSalary(GameProfile_LoL stats, int age)
        {
            return CalculateMarketValue(stats, age) / 20; // Salary is 5% of market value
        }

        public static int CalculateCSPlayerRating(Player player, GameProfile_CS profile)
        {
            double a = Math.Round((profile.Tactics*1.2 + profile.Mechanics*1.2 + profile.Reflexes + profile.Communication + profile.Teamwork + profile.GameSense + profile.Adaptability + profile.Consistency)/8);
            int rating = (int)a;
